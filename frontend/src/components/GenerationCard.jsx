...
export default function GenerationCard({ task, onAddImage, onAddPrompt, onAddToPromptLibrary, onRetry, selectMode, checked, onToggleCheck, wasDraggedRef, showUsername, username, onViewDetail, thumbnailBlurred = false, onToggleThumbnailBlur, masonry = false, nowTs = Date.now(), ...rest }) {
  ...
  return (
    <UnifiedCard
      ...
      bottomNode={task.error && !selectMode ? <div className="absolute bottom-0 left-0 right-0 px-2 py-1.5 bg-gradient-to-t from-red-900/80 to-transparent"><p className="text-red-200 text-xs truncate">{task.error}</p></div> : (prompt ? <div className="absolute bottom-0 left-0 right-0 hidden md:block px-2 py-1.5 bg-gradient-to-t from-black/72 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200"><p className="text-white text-xs truncate">{prompt}</p></div> : null)}
