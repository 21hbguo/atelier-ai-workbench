...
export function CardGridSkeleton({ layoutMode = 'grid', count = 10, label = '加载中...', className = '' }) {
  ...
  return <div className={className}><div className="mb-3 flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}><Loader2 size={14} className="animate-spin" /><span>{label}</span></div><div className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'}>{Array.from({ length: count }).map((_, idx) => <div key={idx} className={useMasonry ? 'card-feed-item-masonry' : ''}><div className="card-feed-skeleton-card" style={{ aspectRatio: idx % 5 === 0 ? '4 / 5' : idx % 3 === 0 ? '3 / 4' : '1 / 1' }}><div className="card-feed-skeleton-shimmer" /></div></div>)}</div></div>
}
...
export default function CardGrid({
  ...
  if (loading && cards.length === 0) return <CardGridSkeleton layoutMode={layoutMode} />
      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={{ position: 'relative', ...((loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined) }}>
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            mediaNode={(() => { const ratio = getMediaRatio(card.width, card.height, '1 / 1'); const text = card.title || card.subtitle || '无提示词'; const len = text.length; const fontSize = len <= 4 ? '2rem' : len <= 8 ? '1.5rem' : len <= 16 ? '1.125rem' : '0.875rem'; const fallback = <div className="w-full flex items-center justify-center p-3" style={{ aspectRatio: ratio, background: 'linear-gradient(135deg, var(--accent)12, var(--accent)20)' }}><p className="text-center font-bold leading-tight line-clamp-4" style={{ color: 'var(--accent)', fontSize }}>{text}</p></div>; return card.thumbUrl ? <ProgressiveImage src={card.thumbUrl} srcSet={card.thumbUrl2x ? `${card.thumbUrl} 1x, ${card.thumbUrl2x} 2x` : undefined} sizes="(min-width: 1024px) 20vw, (min-width: 768px) 25vw, (min-width: 640px) 33vw, 50vw" width={card.width || undefined} height={card.height || undefined} alt="" draggable={false} className={mediaClassName} loading={idx < 8 ? 'eager' : 'lazy'} ratio={ratio} fallback={fallback} /> : fallback })()}
