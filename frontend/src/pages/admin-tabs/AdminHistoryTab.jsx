...
export default function AdminHistoryTab({
  historyTotal, historyQuery, setHistoryQuery,
  historySummary,
  loading,
  history,
  modelLabelMap,
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <SearchInput value={historyQuery} onChange={setHistoryQuery} placeholder="搜索提示词/账号..." />
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 mb-4">
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>总计</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{historySummary.total || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>等待中</div>
          <div className="text-sm font-semibold" style={{ color: '#6b7280' }}>{historySummary.pending || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>排队中</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-warning)' }}>{historySummary.queued || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>生成中</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-warning)' }}>
            {(historySummary.processing || 0) + (historySummary.running || 0) + (historySummary.generating || 0)}
          </div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>processing</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-warning)' }}>{historySummary.processing || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>running</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-warning)' }}>{historySummary.running || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>已完成</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-success)' }}>{historySummary.completed || 0}</div>
        </div>
        <div className="px-3 py-2 rounded-2xl border" style={{ borderColor: 'var(--border-color)' }}>
          <div className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>失败</div>
          <div className="text-sm font-semibold" style={{ color: 'var(--color-error)' }}>{historySummary.failed || 0}</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {history.map(item => {
                    const statusMetaMap = {
                      pending: { c: 'var(--text-secondary)', bg: 'var(--border-color)', l: '等待中' },
                      queued: { c: 'var(--color-warning)', bg: 'color-mix(in srgb, var(--color-warning) 12%, transparent)', l: '排队中' },
                      processing: { c: 'var(--color-warning)', bg: 'color-mix(in srgb, var(--color-warning) 12%, transparent)', l: '生成中' },
                      running: { c: 'var(--color-warning)', bg: 'color-mix(in srgb, var(--color-warning) 12%, transparent)', l: '生成中' },
                      generating: { c: 'var(--color-warning)', bg: 'color-mix(in srgb, var(--color-warning) 12%, transparent)', l: '生成中' },
                      completed: { c: 'var(--color-success)', bg: 'color-mix(in srgb, var(--color-success) 12%, transparent)', l: '已完成' },
                      failed: { c: 'var(--color-error)', bg: 'color-mix(in srgb, var(--color-error) 12%, transparent)', l: '失败' },
                    }
                    ...
                    const duration = item.started_at && item.completed_at
                      ? Math.round((parseBeijing(item.completed_at) - parseBeijing(item.started_at)) / 1000)
                      : null
          ...
          {historyTotal > 20 && (
            <Pagination
              page={historyPage}
              totalPages={Math.ceil(historyTotal / 20)}
