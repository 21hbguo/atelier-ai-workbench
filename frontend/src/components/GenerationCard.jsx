...
export default function GenerationCard({ task, onAddImage, onAddPrompt, onRetry, selectMode, checked, onToggleCheck, showUsername, username, onViewDetail }) {
  ...
  return (
    <UnifiedCard
      checked={checked}
      onClick={() => { if (selectMode) { onToggleCheck?.(); return }; if (isCompleted) onViewDetail?.() }}
      mediaNode={images.length > 0 ? <img src={images[0].thumb} alt="" className="w-full aspect-square object-cover" /> : <div className="w-full aspect-square flex flex-col items-center justify-center gap-3" style={{ background: cfg.bg }}>{isProcessing ? <><div className="w-10 h-10 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: cfg.color, borderTopColor: 'transparent' }} /><span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label} {Math.round(progress)}%</span></> : task.status === 'failed' ? <><span className="text-2xl">!</span><span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label}</span></> : <span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label}</span>}</div>}
      hoverNode={!selectMode && isCompleted ? <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors hidden md:flex items-center justify-center gap-1.5">{prompt && onAddPrompt && <button onClick={(e) => { e.stopPropagation(); onAddPrompt(prompt) }} className="opacity-0 group-hover:opacity-100 transition-opacity px-2.5 py-1.5 rounded-lg text-xs font-medium bg-white/90 text-gray-800 hover:bg-white flex items-center gap-1"><Plus size={12} /> 提示词</button>}{onAddImage && <button onClick={(e) => { e.stopPropagation(); onAddImage(images[0].full) }} className="opacity-0 group-hover:opacity-100 transition-opacity px-2.5 py-1.5 rounded-lg text-xs font-medium bg-white/90 text-gray-800 hover:bg-white flex items-center gap-1"><Image size={12} /> 参考图</button>}<button onClick={handleShare} disabled={shared || sharing} className={`opacity-0 group-hover:opacity-100 transition-opacity px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1 ${shared ? 'bg-green-500/90 text-white cursor-default' : 'bg-white/90 text-gray-800 hover:bg-white'}`}><Share2 size={12} /> {shared ? '已分享' : sharing ? '...' : '广场'}</button><button onClick={handleShareLink} disabled={sharing} className={`opacity-0 group-hover:opacity-100 transition-opacity px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1 bg-white/90 text-gray-800 hover:bg-white`}><Share2 size={12} /> 分享链接</button></div> : null}
      ...
      topLeftNode={showUsername && username && isCompleted && !selectMode ? <div className="absolute top-2 left-2 z-10 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-black/50 backdrop-blur-sm"><span className="text-white text-xs truncate max-w-[80px]">{username}</span></div> : null}
      topRightNode={!isCompleted && !selectMode ? <div className="absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm"><span className="text-white text-xs font-medium">{cfg.label}</span></div> : task.status === 'failed' && onRetry && !selectMode ? <button onClick={(e) => { e.stopPropagation(); onRetry(task.task_id) }} className="absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm text-white text-xs hover:bg-black/70 transition-colors"><RefreshCw size={12} /> 重试</button> : (isCompleted && !selectMode ? <div className="absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm"><span className={`text-xs font-medium ${task.is_permanent ? 'text-green-300' : task.expired ? 'text-red-300' : 'text-yellow-200'}`}>{expiryText}</span></div> : null)}
