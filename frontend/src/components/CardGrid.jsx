  ...
  return (
    <>
      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={{ position: 'relative', ...((loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined) }}>
        ...
        {cards.map((card, idx) => { const ratio = card.width && card.height ? `${card.width} / ${card.height}` : '1 / 1'; const text = card.title || card.subtitle || '无提示词'; const len = text.length; const fontSize = len <= 4 ? '2rem' : len <= 8 ? '1.5rem' : len <= 16 ? '1.125rem' : '0.875rem'; const squareTag = getCardTag(card); const squareTitle = getCardTitle(card); const squareMeta = getCardMeta(card); const squareBody = trimCardText(card.prompt || card.subtitle || card.title || '', 64); const imageNode = card.thumbUrl && !failedUrls.has(card.thumbUrl) ? <div className="card-feed-media-shell" style={{ aspectRatio: ratio }}><img src={card.thumbUrl} srcSet={card.thumbUrl2x ? `${card.thumbUrl} 1x, ${card.thumbUrl2x} 2x` : undefined} sizes="(min-width: 1024px) 20vw, (min-width: 768px) 25vw, (min-width: 640px) 33vw, 50vw" width={card.width || undefined} height={card.height || undefined} alt="" draggable={false} className="card-feed-media" loading={idx < 8 ? 'eager' : 'lazy'} onError={(e) => { e.target.onerror = null; setFailedUrls(prev => new Set(prev).add(card.thumbUrl)) }} /></div> : <div className="w-full flex items-center justify-center p-3" style={{ aspectRatio: ratio, background: 'linear-gradient(145deg,color-mix(in srgb,var(--accent) 22%,transparent),color-mix(in srgb,var(--bg-card) 88%,#fff))' }}><p className="text-center font-bold leading-tight line-clamp-4" style={{ color: 'var(--accent)', fontSize }}>{text}</p></div>; const squareMedia = <div className="px-2.5 pt-2.5"><div className="relative overflow-hidden rounded-[1.35rem] border border-black/6 bg-[var(--bg-ai-bubble)] shadow-[0_18px_40px_rgba(18,30,24,0.12)]">{imageNode}<div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.18),rgba(255,255,255,0.02)_26%,rgba(14,18,17,0.03)_54%,rgba(14,18,17,0.54)_100%)]" /><div className="pointer-events-none absolute inset-x-[14%] top-[8%] h-[18%] rounded-full bg-white/18 blur-2xl" /></div></div>; return <UnifiedCard
            key={card.id}
