...
import { Coins, ArrowUpCircle, ArrowDownCircle, RefreshCw, User, Mail, Gift, Wallet, Upload, Copy } from 'lucide-react'
...
if(!txNo.trim()&&!proofUrl.trim()){setRechargeMsg({type:'error',text:'请至少填写交易单号或上传支付凭证'});return}
setSubmittingRecharge(true)
setRechargeMsg(null)
try{
await pointsAPI.createRechargeRequest({channel:rechargeChannel,amount:rechargeAmount,points:rechargePoints,payer_name:payerName,tx_no:txNo,proof_url:proofUrl,remark})
setTxNo('')
setProofUrl('')
setRemark('')
...
<div className="grid sm:grid-cols-2 gap-2 mb-2"><input type="text" value={payerName} onChange={e=>setPayerName(e.target.value)} placeholder="付款人（选填）" className="px-3 py-2 rounded-lg text-sm outline-none transition-colors" style={{background:'var(--bg-primary)',color:'var(--text-primary)',border:'1px solid var(--border-color)'}} /><input type="text" value={txNo} onChange={e=>setTxNo(e.target.value)} placeholder="交易单号（建议填写）" className="px-3 py-2 rounded-lg text-sm outline-none transition-colors" style={{background:'var(--bg-primary)',color:'var(--text-primary)',border:'1px solid var(--border-color)'}} /></div>
...
<button onClick={handleSubmitRecharge} disabled={submittingRecharge||(!txNo.trim()&&!proofUrl.trim())} className="w-full px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors disabled:opacity-50" style={{background:'var(--accent)'}}>{submittingRecharge?'提交中...':`提交充值申请（¥${rechargeAmount} / ${rechargePoints}积分）`}</button>
