      ...
      <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3" style={(loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined}>
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            topRightNode={<div className="absolute top-2 right-2 flex items-center gap-1.5"><div onClick={(e) => { e.stopPropagation(); onFavorite?.(card.id) }} className="flex items-center justify-center w-7 h-7 rounded-full bg-black/50 backdrop-blur-sm cursor-pointer hover:bg-black/70 transition-colors"><Star size={12} className={card.isFavorited ? 'fill-yellow-400 text-yellow-400' : 'text-white'} /></div><div onClick={(e) => { e.stopPropagation(); onLike?.(card.id) }} className="flex items-center gap-1 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm cursor-pointer hover:bg-black/70 transition-colors"><Heart size={12} className={card.isLiked ? 'fill-red-500 text-red-500' : 'text-white'} />{(card.likesCount > 0 || card.isLiked) && <span className="text-white text-xs">{card.likesCount}</span>}</div></div>}
