      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={{ position: 'relative', ...((loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined) }}>
        ...
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            topLeftNode={showAuthor && Boolean(card.author) ? <button onClick={(e) => { e.stopPropagation(); onAuthorClick?.(card) }} className={`absolute top-2 left-2 hidden md:flex items-center gap-1 h-7 px-2 rounded-full bg-black/50 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity ${onAuthorClick && (card.authorId || card.authorName) ? 'cursor-pointer hover:bg-black/70 transition-colors' : 'cursor-default'}`} disabled={!onAuthorClick || (!card.authorId && !card.authorName)}><User size={12} className="text-white" /><span className="text-white text-xs truncate max-w-[70px]">{card.author}</span></button> : null}
