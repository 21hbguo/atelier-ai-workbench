...
@agent_tool(
    name="image_gen",
    description=(
        "文生图/图生图工具：根据文字描述生成图片。仅当用户明确要求生成图片（生成/画/做一张图、"
        "设计海报/头像/壁纸/插画/LOGO 等视觉成品）时才调用此工具，生成结果直接以图片形式展示在对话中。\n"
        "注意：用户只是想要提示词文案（如「帮我写个提示词」「帮我优化/润色提示词」「帮我描述一下画面」）"
        "而没有要求真正生成图片时，绝对不要调用本工具，直接在回复中给出提示词文本即可。\n"
        "参数规范：\n"
