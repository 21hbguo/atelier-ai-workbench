      ...
      <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3" style={(loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined}>
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            topLeftNode={showAuthor && Boolean(card.author) ? <div className="absolute top-2 left-2 flex items-center gap-1 h-7 px-2 rounded-full bg-black/50 backdrop-blur-sm"><User size={12} className="text-white" /><span className="text-white text-xs truncate max-w-[80px]">{card.author}</span></div> : null}
