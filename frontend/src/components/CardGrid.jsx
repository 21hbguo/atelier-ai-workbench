...
export default function CardGrid({
  ...
  const { selectionRect, dragSelected } = useDragSelection({
    enabled: selectable,
    selected,
    onSelectionChange,
      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={{ position: 'relative', ...((loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined) }}>
        ...
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            selectNode={selectable ? <><div className={`absolute left-2 z-20 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${(selected.has(card.id) || dragSelected.has(String(card.id))) ? 'bg-accent border-accent' : 'bg-[var(--bg-card)]/80 border-[var(--border-color)]'}`} style={{ top: showAuthor && card.author ? '2.5rem' : '0.5rem' }}>{(selected.has(card.id) || dragSelected.has(String(card.id))) && <Check size={12} className="text-white" />}</div>{(selected.has(card.id) || dragSelected.has(String(card.id))) && <div className="absolute inset-0 bg-accent/10 pointer-events-none z-10" />}</> : null}
