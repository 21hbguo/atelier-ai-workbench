...
return <MainLayout><div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="max-w-4xl mx-auto"><div className="flex items-center justify-between mb-4"><h1 className="text-lg font-semibold" style={{color:'var(--text-primary)'}}>通知中心</h1><button onClick={markAll} className="px-3 py-1.5 rounded-lg text-sm font-medium text-white" style={{background:'var(--accent)'}}>全部已读</button></div>{loading?<div className="text-sm" style={{color:'var(--text-secondary)'}}>加载中...</div>:items.length===0?<div className="text-sm" style={{color:'var(--text-secondary)'}}>暂无通知</div>:<div className="space-y-2">{items.map(n=><div key={n.id} className="p-3 rounded-xl border" style={{background:'var(--bg-ai-bubble)',borderColor:'var(--border-color)'}}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="text-sm font-medium" style={{color:'var(--text-primary)'}}>{n.title}</div><div className="text-xs mt-1" style={{color:'var(--text-secondary)'}}>{n.content}</div><div className="text-[11px] mt-1" style={{color:'var(--text-secondary)'}}>{new Date(n.created_at).toLocaleString('zh-CN')}</div></div>{!n.is_read&&<button onClick={()=>markRead(n.id)} className="px-2 py-1 rounded text-xs" style={{color:'var(--accent)'}}>标为已读</button>}</div></div>)}</div>}<Pagination page={page} totalPages={totalPages} onPageChange={setPage} /></div></div></MainLayout>
}
