...
const typeMap={register_bonus:{label:'注册赠送',color:'var(--accent)'},daily_checkin:{label:'每日签到',color:'var(--accent)'},generate_consume:{label:'生成消耗',color:'var(--color-error)'},prompt_optimize:{label:'提示词优化',color:'var(--color-error)'},image_expire_extend:{label:'延长有效期',color:'var(--color-error)'},generate_refund:{label:'生成退款',color:'var(--color-success)'},redeem_code:{label:'积分发放',color:'var(--accent)'},admin_grant:{label:'管理员调整',color:'#8B7BA8'},migration:{label:'历史补偿',color:'var(--accent)'},migration_bonus:{label:'历史补偿',color:'var(--accent)'},recharge_pending:{label:'捐赠待审核',color:'var(--color-warning)'},recharge_refund:{label:'积分回退',color:'var(--color-error)'}}
...
const statusMap={pending:{label:'待审核',color:'var(--color-warning)'},approved:{label:'已发放',color:'var(--color-success)'},rejected:{label:'未通过',color:'var(--color-error)'},refunded:{label:'已回退',color:'var(--color-error)'}}
const tabList=[{key:'records',label:'积分记录'},{key:'donate',label:'捐赠获取积分'},{key:'redeem',label:'兑换码'}]
...
useEffect(()=>{
api.get('/config').then(({data})=>{const packages=Array.isArray(data.recharge_packages)&&data.recharge_packages.length?data.recharge_packages:defaultRechargePackages;setPayConfig({wechat_pay_qr_url:data.wechat_pay_qr_url||'',alipay_pay_qr_url:data.alipay_pay_qr_url||'',donation_contact:data.donation_contact||'',manual_recharge_notice:data.manual_recharge_notice||'支持 Atelier 持续承担模型、图床与服务器成本。你可自愿捐赠支持平台运行，审核通过后按页面公示档位赠送对应感谢积分。请备注账号并上传支付凭证，捐赠完成后不支持退款。'});setRechargePackages(packages);setPackageIdx(0);setRechargeAmount(Number(packages[0]?.amount||defaultRechargePackages[0].amount));setRechargePoints(Number(packages[0]?.points||defaultRechargePackages[0].points))}).catch(()=>{})
},[])
...
setRechargeMsg({type:'success',text:'捐赠凭证上传成功'})
}catch(err){
setRechargeMsg({type:'error',text:err.message||'捐赠凭证上传失败'})
}finally{
...
if(!proofUrl.trim()){setRechargeMsg({type:'error',text:'请上传捐赠凭证'});return}
...
setRemark('')
setPayerName('')
setRechargeMsg({type:'success',text:'捐赠凭证已提交，审核通过后自动发放积分'})
fetchData(page)
}catch(err){
...
<div className="flex flex-wrap items-center gap-2 mb-2"><label className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium cursor-pointer hover:bg-bg-hover" style={{color:'var(--text-primary)',border:'1px solid var(--border-color)'}}><Upload size={14} />{uploadingProof?'上传中...':'上传捐赠凭证'}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handleUploadProof} /></label>{proofUrl&&<button type="button" onClick={()=>setProofLightbox(true)} className="text-xs underline" style={{color:'var(--accent)'}}>查看已上传凭证</button>}</div>
<button onClick={handleSubmitRecharge} disabled={submittingRecharge||!proofUrl.trim()} className="w-full px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors disabled:opacity-50" style={{background:'var(--accent)'}}>{submittingRecharge?'提交中...':`提交捐赠凭证（¥${rechargeAmount} / ${rechargePoints}积分）`}</button>
...
<div className="p-4 rounded-xl border" style={{background:'var(--bg-ai-bubble)',borderColor:'var(--border-color)'}}>
<div className="flex items-center gap-2 mb-3"><HeartHandshake size={16} style={{color:'var(--accent)'}} /><span className="text-sm font-medium" style={{color:'var(--text-primary)'}}>捐赠说明</span></div>
...
<p>捐赠属于个人自愿支持行为，不构成商品购买或预付储值，捐赠完成后不支持退款，请按需理性支持。</p>
</div>
</div>
</div>
)}
...
<p className="text-xs mb-3" style={{color:'var(--text-secondary)'}}>输入已发放的兑换码，领取对应积分。</p>
...
<button onClick={handleRedeem} disabled={redeemLoading||!redeemCode.trim()} className="px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors disabled:opacity-50" style={{background:'var(--accent)'}}>{redeemLoading?'领取中...':'领取'}</button>
</div>
