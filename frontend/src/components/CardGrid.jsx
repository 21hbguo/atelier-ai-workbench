      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={{ position: 'relative', ...((loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined) }}>
        ...
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            mediaNode={(() => { const ratio = card.width && card.height ? `${card.width} / ${card.height}` : '1 / 1'; const text = card.title || card.subtitle || '无提示词'; const len = text.length; const fontSize = len <= 4 ? '2rem' : len <= 8 ? '1.5rem' : len <= 16 ? '1.125rem' : '0.875rem'; const mediaInner = card.thumbUrl && !failedUrls.has(card.thumbUrl) ? (useMasonry ? <img src={card.thumbUrl} srcSet={card.thumbUrl2x ? `${card.thumbUrl} 1x, ${card.thumbUrl2x} 2x` : undefined} sizes="(min-width: 1024px) 20vw, (min-width: 768px) 25vw, (min-width: 640px) 33vw, 50vw" width={card.width || undefined} height={card.height || undefined} alt="" draggable={false} className={mediaClassName} loading={idx < 8 ? 'eager' : 'lazy'} onError={(e) => { e.target.onerror = null; setFailedUrls(prev => new Set(prev).add(card.thumbUrl)) }} /> : <div className="card-feed-media-shell" style={{ aspectRatio: ratio }}><img src={card.thumbUrl} srcSet={card.thumbUrl2x ? `${card.thumbUrl} 1x, ${card.thumbUrl2x} 2x` : undefined} sizes="(min-width: 1024px) 20vw, (min-width: 768px) 25vw, (min-width: 640px) 33vw, 50vw" width={card.width || undefined} height={card.height || undefined} alt="" draggable={false} className={mediaClassName} loading={idx < 8 ? 'eager' : 'lazy'} onError={(e) => { e.target.onerror = null; setFailedUrls(prev => new Set(prev).add(card.thumbUrl)) }} /></div>) : <div className="w-full flex items-center justify-center p-3" style={{ aspectRatio: ratio, background: 'linear-gradient(135deg, var(--accent)12, var(--accent)20)' }}>{(() => <p className="text-center font-bold leading-tight line-clamp-4" style={{ color: 'var(--accent)', fontSize }}>{text}</p>)()}</div>; return isSquareMode ? <div className="pb-1"><div className="overflow-hidden rounded-2xl">{mediaInner}</div></div> : mediaInner })()}
            ...
            footerNode={isSquareMode ? <div onClick={(e) => e.stopPropagation()} className="flex items-center px-2.5 py-1.5 bg-[var(--bg-card)]"><div className="flex w-full items-center justify-between gap-1.5 flex-shrink-0">{Boolean(card.prompt) && <button onClick={(e) => { e.stopPropagation(); onUsePrompt?.(card) }} className="h-7 px-2 rounded-full text-[11px] font-medium border" style={{ color: 'var(--text-primary)', borderColor: 'var(--border-color)', background: 'var(--bg-primary)' }}><span>做同款</span></button>}{showLike && onLike ? <button onClick={async (e) => { e.stopPropagation(); await onLike(card.id) }} className="flex items-center gap-1 px-2 h-7 rounded-full border" style={{ color: card.isLiked ? '#ef4444' : 'var(--text-secondary)', borderColor: 'var(--border-color)', background: 'var(--bg-primary)' }}><Heart size={11} className={card.isLiked ? 'fill-red-500 text-red-500' : ''} /><span className="text-[11px] leading-none">{card.likesCount || 0}</span></button> : null}{onFavorite ? <button onClick={async (e) => { e.stopPropagation(); await onFavorite(card.id) }} className="flex items-center justify-center w-7 h-7 rounded-full border" style={{ color: card.isFavorited ? '#f59e0b' : 'var(--text-secondary)', borderColor: 'var(--border-color)', background: 'var(--bg-primary)' }}><Star size={11} className={card.isFavorited ? 'fill-yellow-400 text-yellow-400' : ''} /></button> : null}</div></div> : null}
          />
        ))}
