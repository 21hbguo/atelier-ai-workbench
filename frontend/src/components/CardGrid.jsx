      ...
      <div ref={gridRef} className={useMasonry ? 'card-feed-masonry' : 'card-feed-grid'} style={(loading || paging) && gridMinHeight > 0 ? { minHeight: `${gridMinHeight}px` } : undefined}>
        ...
        {cards.map((card, idx) => (
          <UnifiedCard
            ...
            topLeftNode={showAuthor && Boolean(card.author) ? <button onClick={(e) => { e.stopPropagation(); onAuthorClick?.(card) }} className={`absolute top-2 left-2 flex items-center gap-1 h-7 px-2 rounded-full bg-black/50 backdrop-blur-sm ${onAuthorClick && card.authorId ? 'cursor-pointer hover:bg-black/70 transition-colors' : 'cursor-default'}`} disabled={!onAuthorClick || !card.authorId}><User size={12} className="text-white" /><span className="text-white text-xs truncate max-w-[70px]">{card.author}</span></button> : null}
