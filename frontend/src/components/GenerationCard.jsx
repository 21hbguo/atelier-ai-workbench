...
import ProgressiveImage, { getMediaRatio } from './ProgressiveImage'
...
export default function GenerationCard({ task, onAddImage, onAddPrompt, onAddToPromptLibrary, onRetry, selectMode, checked, onToggleCheck, wasDraggedRef, showUsername, username, onViewDetail, thumbnailBlurred = false, onToggleThumbnailBlur, masonry = false, nowTs = Date.now(), ...rest }) {
  ...
  const completedRatio = getMediaRatio(images[0]?.width, images[0]?.height, '1 / 1')
  ...
  return (
    <UnifiedCard
      ...
      mediaNode={isCompleted && images.length > 0 ? <div className="relative"><ProgressiveImage src={images[0].thumb} srcSet={images[0].thumb2x ? `${images[0].thumb} 1x, ${images[0].thumb2x} 2x` : undefined} sizes="(min-width: 1024px) 20vw, (min-width: 768px) 25vw, (min-width: 640px) 33vw, 50vw" width={images[0].width || undefined} height={images[0].height || undefined} alt="" draggable={false} className={`${mediaClassName} ${thumbnailBlurred ? 'scale-105 blur-md' : ''}`} ratio={completedRatio} fallback={<div className="w-full flex items-center justify-center text-xs" style={{ aspectRatio: completedRatio, background: 'var(--border-color)', color: 'var(--text-secondary)' }}>加载失败</div>} />{thumbnailBlurred && <div className="absolute inset-0 bg-white/10 backdrop-blur-md" />}</div> : <div className="w-full aspect-square flex flex-col items-center justify-center gap-3" style={{ background: cfg.bg }}>{isProcessing ? <><div className="w-10 h-10 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: cfg.color, borderTopColor: 'transparent' }} /><span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label} {Math.round(progress)}%</span></> : task.status === 'failed' ? <><span className="text-2xl">!</span><span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label}</span></> : <span className="text-xs font-medium" style={{ color: cfg.color }}>{cfg.label}</span>}</div>}
