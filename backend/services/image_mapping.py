...
class ImageUrlMapping:
    ...
    def get_url(cls, local_path: str) -> Optional[str]:
        ...
        with get_db() as conn:
            row = conn.execute("SELECT url FROM image_mappings WHERE local_path = %s", (abs_path,)).fetchone()
            return row["url"] if row else None

    @classmethod
    def get_url_by_hash(cls, content_hash: str) -> Optional[str]:
        with get_db() as conn:
            row = conn.execute("SELECT url FROM image_mappings WHERE content_hash = %s", (content_hash,)).fetchone()
            return row["url"] if row else None
    ...
    def save_url(cls, local_path: str, url: str, content_hash: str = "", delete_token: str = "") -> Optional[str]:
        ...
        with get_db() as conn:
            ...
            conn.execute(
                "INSERT INTO image_mappings (local_path, url, upload_time, content_hash, delete_token) VALUES (%s, %s, %s, %s, %s) ON CONFLICT(local_path) DO NOTHING",
